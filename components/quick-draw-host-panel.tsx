...
export function QuickDrawHostPanel({ accessToken, roomCode, onEnded }: { accessToken: string; roomCode: string; onEnded?: () => void }) {
  ...
  if (session.state.phase === 'drawing') return <section className="panel quick-draw-live-panel">
    <div className="live-line"><span className="live-dot" /> {paused ? `PAUSED · TURN ${turnNumber}/${session.config.artistTurns}` : `QUICK DRAW · TURN ${turnNumber}/${session.config.artistTurns}`}</div>
    {error && <div className="notice warning" role="alert">{error}</div>}
    {paused && <div className="notice warning">Drawing and guessing are frozen. Resume continues with the same remaining time.</div>}
    <div className="section-heading"><div><div className="eyebrow">Artist</div><h2>{session.artist.nickname}</h2></div><div className="bingo-callout"><strong>{remaining}s</strong><span>{paused ? 'frozen' : 'remaining'}</span></div></div>
    ...
    <button className="btn secondary full-width" disabled={busy} onClick={() => void togglePause()}>{paused ? 'Resume turn' : 'Pause turn'}</button>
    <button className="btn primary full-width" disabled={busy || paused || remaining > 0} onClick={() => void run(() => finishQuickDrawRoundClient(accessToken, roomCode))}>{paused ? 'Paused' : remaining > 0 ? `Turn ends in ${remaining}s` : 'Reveal word & score turn'}</button>
    <button className="btn secondary full-width" disabled={busy || paused} onClick={() => void run(() => finishQuickDrawRoundClient(accessToken, roomCode, true))}>Skip turn & reveal</button>
  ...
  return <section className="panel quick-draw-live-panel">
    ...
    <button className="btn primary full-width" disabled={busy || paused} onClick={() => void run(() => nextQuickDrawRoundClient(accessToken, roomCode))}>{isLast ? 'Finish & show results' : 'Next artist'}</button>
