...
export function MajorityMatchHostPanel({ accessToken, roomCode, onEnded }: { accessToken: string; roomCode: string; onEnded?: () => void }) {
  ...
  if (session.state.phase === 'answering') return <section className="panel majority-live-panel">
    <div className="live-line"><span className="live-dot" /> MAJORITY MATCH · QUESTION {questionNumber}/{session.config.questionCount}</div>
    {error && <div className="notice warning" role="alert">{error}</div>}
    <div className="bingo-callout"><strong>{remaining}s</strong><span>answer time remaining</span></div>
    ...
    <button className="btn primary full-width" disabled={busy || remaining > 0} onClick={() => void run(() => revealMajorityClient(accessToken, roomCode))}>{remaining > 0 ? `Voting closes in ${remaining}s` : 'Reveal majority'}</button>
    <button className="btn secondary full-width" disabled={busy} onClick={() => void run(() => revealMajorityClient(accessToken, roomCode, true))}>Close voting early & reveal</button>
  ...
  return <section className="panel majority-live-panel">
    <div className="live-line"><span className="live-dot" /> RESULT · QUESTION {questionNumber}/{session.config.questionCount}</div>
    <h2>{session.state.currentQuestion.prompt}</h2>
    ...
    <button className="btn primary full-width" disabled={busy} onClick={() => void run(() => nextMajorityClient(accessToken, roomCode))}>{isLast ? 'Finish & show results' : 'Next question'}</button>
