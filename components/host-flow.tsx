...
export function HostFlow({ onExit }: { onExit: () => void }) {
  ...
  async function startGame() { await patchRoom({ status: 'playing' }); setStep(4); }
  async function finishGame() { await patchRoom({ status: 'results' }); setStep(5); }
  ...
  return (
    <main className="role-shell" data-app="host">
      ...
      {step === 0 && <section className="workspace narrow">
        <div className="eyebrow">Time first</div><h1>How much time does the group have?</h1>
      ...
      {step === 3 && liveRoom && <section className="workspace two-column">
        <div className="panel room-status">
          <div className="live-line"><span className="live-dot" /> {realtimeStatus === 'SUBSCRIBED' ? 'LIVE ROOM' : 'CONNECTING'}</div><div className="room-code">{liveRoom.join_code}</div>
          ...
          <div className="meta"><span className="pill">Online {snapshot?.counts.online ?? 0}</span><span className="pill">Reconnecting {snapshot?.counts.reconnecting ?? 0}</span><span className="pill">Spectators {snapshot?.counts.spectators ?? 0}</span></div>
          <div className="participant-list">{snapshot?.participants.map((participant) => <div className="control-row" key={participant.id}><span>{participant.nickname} · {participant.role}</span><strong>{participant.online ? '● Online' : '○ Reconnecting'}</strong></div>)}</div>
        </div>
        <aside className="panel status-panel">
          <h2>Lobby controls</h2><div className={`readiness ${readiness.state}`}><strong>{readiness.canStart ? '✓' : '!'} {readiness.state === 'ready' ? 'Ready' : 'Room status'}</strong><span>{readiness.message}</span></div>
          ...
          <button className="btn primary full-width" disabled={busy || !readiness.canStart} onClick={() => void startGame()}>Start game</button>
        </aside>
        <div className="primary-row full"><button className="btn secondary" onClick={() => setStep(2)}>← Edit setup</button></div>
      </section>}

      {step === 4 && liveRoom && <section className="workspace two-column">
        <div className="panel play-stage"><div className="live-line"><span className="live-dot" /> LIVE · {game.name}</div><h1>{liveRoom.status === 'paused' ? 'Game paused' : 'Round in progress'}</h1><p className="support">Game-specific content plugs into this same server-authoritative room, score, moderation, and reconnect infrastructure.</p><div className="demo-board"><span>{minutes}:00</span><strong>{activePlayers} players</strong></div></div>
        <aside className="panel status-panel"><h2>Host controls</h2><button className="btn primary full-width" disabled={busy} onClick={() => void patchRoom({ status: liveRoom.status === 'paused' ? 'playing' : 'paused' })}>{liveRoom.status === 'paused' ? 'Resume' : 'Pause'}</button><button className="btn secondary full-width" disabled>Skip · game engine next</button><button className="btn secondary full-width" disabled>Participant moderation · next</button><button className="btn secondary full-width" disabled>Leaderboard visibility · next</button><button className="btn danger full-width" disabled={busy} onClick={() => void finishGame()}>End game</button></aside>
      </section>}

      {step === 5 && <section className="workspace narrow results-stage">
        <div className="eyebrow">Results</div><h1>Round complete</h1><div className="podium"><div className="podium-place second">2<span>Pending game engine</span></div><div className="podium-place first">1<span>Pending game engine</span></div><div className="podium-place third">3<span>Pending game engine</span></div></div>
        <p className="support">The room lifecycle is live; score and ranking data will be populated by the Release 1 server game engines in the next milestone.</p>
        <div className="primary-row"><button className="btn primary" disabled={busy} onClick={() => void replay()}>Replay</button><button className="btn secondary" onClick={() => setStep(1)}>Change game · keep room</button><button className="btn danger" disabled={busy} onClick={() => void endRoom()}>End room</button></div>
      </section>}
    </main>
  );
}
