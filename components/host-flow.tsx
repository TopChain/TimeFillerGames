...
export function HostFlow({ onExit }: { onExit: () => void }) {
  ...
  return <main className="role-shell" data-app="host">
    ...
    {step === 3 && liveRoom && <section className="workspace two-column">
      <div className="panel room-status"><div className="live-line"><span className="live-dot" /> {realtimeStatus === 'SUBSCRIBED' ? 'LIVE ROOM' : 'CONNECTING'}</div><div className="room-code">{liveRoom.join_code}</div><p className="support">Players can join without creating a visible account. Their temporary authenticated session is used only to authorize this room and recover their seat.</p><div className="share-grid"><RoomQrCode roomCode={liveRoom.join_code} /><div><strong>Room language</strong><span>{LOCALES.find((locale) => locale.id === liveRoom.room_language)?.label}</span></div></div><div className="lobby-count"><strong>{snapshot?.counts.active ?? 0}</strong><span>active participants</span></div><div className="meta"><span className="pill">Ready {snapshot?.counts.ready ?? 0}/{snapshot?.counts.active ?? 0}</span><span className="pill">Online {snapshot?.counts.online ?? 0}</span><span className="pill">Reconnecting {snapshot?.counts.reconnecting ?? 0}</span><span className="pill">Spectators {snapshot?.counts.spectators ?? 0}</span></div><div className="participant-list">{snapshot?.participants.map((participant) => <div className="control-row" key={participant.id}><span>{participant.nickname} · {participant.role}</span><strong>{participant.ready ? '✓ Ready' : participant.online ? '● Online' : '○ Reconnecting'}</strong></div>)}</div></div>
