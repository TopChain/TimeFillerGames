...
export function PeopleBingoHostPanel({ accessToken, roomCode, onEnded }: { accessToken: string; roomCode: string; onEnded?: () => void }) {
  ...
  const canDraw = session.state.phase !== 'ended' && (selectionComplete || remaining <= 0);

  return <section className="panel people-bingo-live-panel">
    <div className="live-line"><span className="live-dot" /> PEOPLE BINGO 5×5</div>
    {error && <div className="notice warning" role="alert">{error}</div>}

    {session.state.phase === 'card-selection' && <>
      <div className="section-heading"><div><div className="eyebrow">Card selection</div><h2>{session.selection.selected}/{session.selection.total} cards locked</h2></div><div className="bingo-callout"><strong>{remaining}s</strong><span>remaining</span></div></div>
    ...
    <button className="btn primary full-width" disabled={busy || !canDraw} onClick={() => void draw()}>{busy ? 'Drawing…' : !canDraw ? `Card choice closes in ${remaining}s` : 'Draw Next Participant'}</button>
