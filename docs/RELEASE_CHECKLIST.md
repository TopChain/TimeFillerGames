...
Source of truth: Product Plan v1.0 + Brand & Product CI Guidelines v1.0. This file distinguishes completed code gates from external publication/QA gates.
...
- [x] Landing shell, Host setup/lobby/results, Host game-control panels, and Player Release 1 game/status/results use six-language dictionaries.
...
- [x] Co-host remains an active player across all Release 1 games.
...
- [x] Web/server CI: TypeScript + rule/content/localization tests + production Next.js build.
- [x] Mobile CI: bundled Vite/Capacitor client build.
- [x] Android native CI generates platform, verifies min SDK 26 / compile+target API 36, deep link, HTTPS-only transport, Release 1 version, branded launcher asset, debug APK and release AAB build.
- [x] iOS validation workflow exists: generated platform, camera permission, deep link, Release 1 version, unsigned simulator Xcode build.
...
- [ ] Run the manual macOS/iOS simulator workflow (connector cannot dispatch workflow_dispatch).
...
- [x] Public `/privacy`, `/privacy-policy`, `/terms`, and `/support` routes exist; legal/account/support links are exposed in web and native shells.
...
- [x] Store metadata/privacy/data-safety working drafts versioned in `docs/`.
- [x] Privacy/account-deletion paths implemented for Apple/Google review requirements.
- [ ] Production HTTPS domain/hosting deployment and environment secrets. Vercel connector currently cannot create the project because its deployment action exposes an invalid/missing payload schema.
...
- [ ] Google Play Console app, Play App Signing/keystore, testing track and upload.
